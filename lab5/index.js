...
app.get("/", async function (req, res) {
  let sql = `SELECT authorId, firstName, lastName 
   ...
   let rows = await executeSQL(sql);
    res.render('index', {"authorDB": rows}); // index.ejs route and author db results
});//dbTest
...
app.get("/searchByCategory", async function (req, res) { // request, and response
  let cat = req.query.category;
    let sql = `SELECT DISTINCT quote, authorId, firstName, lastName
  FROM q_quotes
  NATURAL JOIN q_authors
  WHERE category LIKE ?
  ORDER BY quote`; // ? is param var, prolly in order
